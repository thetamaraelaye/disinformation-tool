...
//api call to get groupsa that discuss about keyword --- "natural remedies for cancer"
let options = {
  method: "POST",
...
request(options, function (error, response) {
 ...
 console.log(groupsData);
  ...
  try {
   // console.log(parsedGroupData, "parsedGroupData"); //data is parsed incorrecgly due to its comples structure
    // Loop through results and log each object
    parsedGroupData.data.results.forEach((result, index) => {
    const cleanedResults = parsedGroupData.data.results.map((result, index) => {
        ...
        const cleanedData = {
          ...
          description: result.description ? result.description.trim() : `No Description`, // Remove any extra spaces from description, if present
